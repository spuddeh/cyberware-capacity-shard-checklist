...
<#
.SYNOPSIS
    One-time backfill of historical GitHub Releases for Cyberware Capacity Shard Checklist.
...
.DESCRIPTION
    ...
    --latest=false so none of them steals "Latest" from the next real release (ccsc-v2.0.0+).
...
# 1.1.0 was an internal 0-Engine temp build, never uploaded, so it is omitted.
$releases = @(
    @{ artifact='ccsc'; version='1.0.0'; zip='Cyberware_Capacity_Shard_Checklist_v1.0.0.zip'; changelog=$changelog }
)
