...
| --- | --- | --- | --- |
| `ccsc` | Cyberware Capacity Shard Checklist | 27806 | main |
...
2. **File ID (manifest).** On the mod page, open the **Files** tab > **API Info** (or the Manage
   Files edit menu) to get the file ID, then replace the `REPLACE_WITH_CCSC_FILE_ID` placeholder in
...
2. Create a GitHub Release whose **tag** follows `<artifact>-v<version>`:
   ```pwsh
   gh release create ccsc-v2.0.0 --title "Cyberware Capacity Shard Checklist v2.0.0" --notes "..."
   ```
...
3. On publish, the workflow:
   ...
   - stages `contentDir` -> `installDir` and zips it as `<fileBaseName>_v<version>.zip` (e.g. `Cyberware_Capacity_Shard_Checklist_v2.0.0.zip`),
